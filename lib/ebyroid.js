 ...
 */
async function internalConvertF(text, vr, option) {
...
class Ebyroid {
   ...
   */
  async convertEx(text, voiceroidName, option) {
    if (this.using === null) {
    ...
    if (!needsLibraryReload(vr)) {
      ...
      return internalConvertF.call(this, text, vr, option);
    }
  ...
  convert(text, option) {
    ...
    if (needsLibraryReload(this.using)) {
      ...
      return this.convertEx(text, this.using.name, option);
    }
    return internalConvertF.call(this, text, this.using, option);
  }
