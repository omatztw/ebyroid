...
const native = require('../dll/ebyroid.node'); // eslint-disable-line node/no-unpublished-require
 ...
 */
async function internalConvertF(text, vr, speed, pitch, range) {
...
class Ebyroid {
   ...
   */
  async convertEx(text, voiceroidName, speed, pitch, range) {
    if (this.using === null) {
    ...
    if (!needsLibraryReload(vr)) {
      ...
      return internalConvertF.call(this, text, vr, speed, pitch, range);
    }
   ...
   * @returns {Promise<WaveObject>} object that consists of a raw PCM buffer and format information
   */
  convert(text, speed, pitch, range) {
    validateOpCall(this);
    if (needsLibraryReload(this.using)) {
      debug('convert() escalates to convertEx()');
      return this.convertEx(text, this.using.name, speed, pitch, range);
    }
    return internalConvertF.call(this, text, this.using, speed, pitch, range);
  }
