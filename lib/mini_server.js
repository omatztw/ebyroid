...
async function onGetAudioFileF(req, res, params) {
  ...
  try {
    /** @type {WaveObject} */ let pcm;
    const name = params.get('name');
    const speed = Number(params.get('speed')) || 1.0;
    const pitch = Number(params.get('pitch')) || 1.0;
    const range = Number(params.get('range')) || 1.0;

    if (name && name !== this.defaultName) {
      pcm = await this.ebyroid.convertEx(text, name, speed, pitch, range);
    } else {
      pcm = await this.ebyroid.convert(text, speed, pitch, range);
    }
...
async function onRequestF(req, res) {
  ...
  switch (pathname) {
    ...
    case '/audiofile':
      console.log('call audiofile with', url.searchParams);
